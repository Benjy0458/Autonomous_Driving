...
class World:
        ...
        try:
            ...
            while running:
                ...
                for event in pygame.event.get():  # Decide whether to terminate the simulation
                    ...
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_p:
                            # Save a screenshot of the simulation window if p-key is pressed
                            pygame.image.save(self.display_surface, f"{time.time()}.jpeg")

                # Vary the traffic density as the simulation progresses
                ...
                if live_data and self.agent:
                    ...
                    if not i:
                        queue.put((elapsed_time, self.agent.xte, self.agent.phi,
                                   self.agent.delta, self.agent.velocity, self.agent.acc))

                clock.tick_busy_loop(FPS * TIME_GAIN)  # Ensure program maintains desired frame rate
        except Exception as err:
            logger.warning(f"Encountered {err}. Exiting the simulation...")
        finally:
            if self.TM:
            ...
            if live_data:
                ...
                live_plot.kill()
                logger.debug("Successfully closed the live plot.")

    def caption(self, elapsed_time: float, clock: pygame.time.Clock) -> None:
        """Updates the Pygame window caption."""
    ...
    def draw_window(self) -> None:
        ...
        self.agent and draw_radar(horizontal=False)  # Draw lines on the screen to indicate vehicles being tracked
        # Draw traffic
    ...
    def init_pygame() -> pygame.Surface:
        ...
        x_pos, y_pos = 0, 30  # Position of the top level corner of the window
...
class TrafficManager:
    ...
    def __init__(self):
        self.ordered_vehicles = [[], [], [], [], [], []]  # Keeps track of the vehicles currently in the window
...
class Vehicle:
    ...
    def __init__(self, vehicle_type: str = "car"):
        match vehicle_type:
            case "car":
                ...
                self.max_velocity = 1.1 * scene.SPEED_LIMIT if self.driving_style == "aggressive" else scene.SPEED_LIMIT
    ...
    def update_position(self):
        """Updates the position of the vehicle on the window."""
        def get_front_car():
            """Finds the closest car in front and determines the appropriate acceleration.
            ...
            if world.agent and world.agent.lane == self.lane and world.agent.x_pos > self.x_pos:
                ...
                acc = self.speed_controller.speed_control.send(
                    [self.x_velocity, 1000, LANE_VELOCITIES[self.lane]]) * 2.237

            return acc

        acc = get_front_car()  # Update vehicle's acceleration in mph/s

        self.x_velocity += acc * (1 / FPS)  # Update velocity
...
class Agent:
    ...
    max_delta = 24  # The maximum steering angle (deg)
    ...
    def __init__(self, path_planner: type[Astar or HybridAstar] = HybridAstar, speed_controller=IDM):
        ...
        self.lateral_controller = OptimalControl(self.length / SCALE)
    ...
    class BtData:
        """A class of data to send to the agent object."""
    ...
    def reset(self):
        ...
        with open("data.txt", mode="w") as file:
            file.write(f'{self.terminal_count} scenarios simulated with {self.n_hits} collisions.')
    ...
    def update_position(self, vehicles):
        ...
        def behavioural_planner():
            """Sends and retrieves latest data from the behavior decision-making algorithm."""
        ...
        def motion_planner():
            """Get control directives from the motion planner."""
            ...
            def adaptive_cruise_control():
                ...
                if self.sensor.radar.closest_cars[0]:
                    ...
                    self.acc = self.speed_controller.speed_control.send(
                        [self.velocity, 1000, self.max_speed]) * 2.237

            def lateral_control():
                """Determine required steering angle of the agent."""
                if self.path:
                    try:
                        # Get desired y-position and heading
                        (self.traj_y_pos, self.traj_heading) = self.path[ceil(self.x_pos)]
                    except KeyError:
                        pass
                    finally:
                ...
                def crosstrack_error():
                    """Returns the current cross-track error."""
                    X_xte = self.x_pos
                    Y_xte = self.traj_y_pos
                    # The magnitude of the cross-track error
                    xte = sqrt((self.x_pos - X_xte) ** 2 + (self.y_pos - Y_xte) ** 2)
                ...
                def heading_error():
                    """Returns the current heading error."""
                ...
                def calculate_steering_angle():
                    """Calculate the required steering angle using optimal controller."""
                    ...
                    max_delta = self.max_delta * 2 * pi / 360  # Convert to radians
        ...
        def update_vehicle_state():
            def update_heading_angle():
                """Calculate the new heading angle of the vehicle."""
                # Yaw rate = Velocity / Radius
                # Steering angle = length / Radius
                # => Radius = length / steering angle
                try:
                    # Handle edge case of zero steering angle
                    if self.delta == 0:
                        yaw_rate = 0
                    else:
    ...
    def get_path(path_planner, send_queue, return_queue):
        """Generator that repeatedly requests new paths from the path planner.
        Description:
...
class Sensor:
    """A class that models a vehicle sensor system."""
...
class Collision:
    """A class that represents a simple collision sensor."""
    def __init__(self, agent):
        self.agent = agent
...
class Radar:
    """A class that models a simple radar sensor."""
    sensor_range = 250
    ...
    def observation(self, vehicles: [[Vehicle]]) -> [[Vehicle], [int]]:
        ...
        def index(x):
            """A helper function that maps a vehicles relative lane to its index in the car list
        ...
        def cond(v):
            """A helper function that determines the direction of adjacent vehicles."""
