...
def animate(i, ax, queue, t=[], xte=[], phi=[], delta=[], vel=[], acc=[], data=[]):
    ...
    ax1.plot(t, delta, c='r', label='delta', linewidth=0.2)
...
def live_graph2(queue, close):
    ...
    ani = animation.FuncAnimation(fig, animate, interval=1000, fargs=(ax, queue), cache_frame_data=False)  # Update the figure window every 10 ms.
    plt.show(block=False)  # Display the plots
